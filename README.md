...
The verified leaderboard boundary, proposed run flow, and open ranking decisions are tracked in [the solo leaderboard integration draft](docs/solo-leaderboard-contract.md). The proposed game-owned API has an [OpenAPI contract](contracts/run-service.openapi.yaml); `src/game/rankedRunVerifier.js` contains the pure replay and timing checks for a future trusted service. No run API or public score submission is deployed yet.
