...
Read [how the game and its movement rules work](docs/blog/nova-orbit-jump-how-it-moves.md) for a player-friendly overview.
...
The ranking rule is weekly-only for guests and weekly plus all-time for Hearso account holders. The game-owned run API, trusted replay, account launch-ticket verifier, and score-delivery path exist locally; they have not been deployed for public ranking.
...
`src/game/runServiceClient.js` is the browser adapter for the game-owned run API. It starts a run, submits a completed transcript with the in-memory run bearer token, and reads the eventual rank state. `src/game/guestCredentialStore.js` keeps only the week-scoped guest credential in private browser storage and drops it after expiry. No browser code receives the leaderboard submission key.
Set `VITE_RUN_API_BASE_URL` to the game-owned HTTPS endpoint ending in `/v1` to enable guest ranked play. The start screen then asks new guests for a public name and country, starts a server run before gameplay, and shows a pending or confirmed weekly rank after a verified campaign win. Returning guests reuse their current-week credential. An unavailable service leaves an explicit local-play choice. Without the variable, the game remains local-only. The backend stack has not been deployed, so this setting is not ready for a production URL yet. A single-use account launch ticket can be verified locally, but the Hearso account handoff is not connected or deployed.
