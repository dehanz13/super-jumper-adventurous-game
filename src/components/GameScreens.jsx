...
export function StartScreen({ onStart, onEnterEditor }) {
  return (
    <div className="absolute inset-0 bg-[#5C94FC] flex flex-col items-center justify-start sm:justify-center overflow-y-auto py-2 sm:py-0" style={{ fontFamily: 'monospace' }}>
      <div className="mb-2 sm:mb-8 text-center">
        <div className="text-[#F8D830] text-base sm:text-2xl font-bold sm:mb-2" style={{ textShadow: '3px 3px 0 #C84C0C' }}>SUPER</div>
        <div className="text-[#E52521] text-2xl sm:text-6xl font-black mb-1" style={{ textShadow: '4px 4px 0 #000, 6px 6px 0 #C84C0C', letterSpacing: '4px' }}>
          MARIO BROS.
        </div>
        <div className="text-white text-xs sm:text-lg sm:mt-4" style={{ textShadow: '2px 2px 0 #000' }}>CLONE EDITION</div>
      </div>

      <div className="text-white text-base sm:text-xl font-bold mb-2 sm:mb-4 cursor-pointer hover:text-[#F8D830] transition-colors"
        onClick={() => onStart(1)}
      ...
      <div className="bg-black/50 p-2 sm:p-4 rounded-lg mb-2 sm:mb-4 relative z-10">
        ...
        <div className="flex flex-col gap-2 sm:gap-4 items-center">
          <div className="flex gap-2 sm:gap-4">
            {[1, 2, 3].map(lvl => (
              <button key={lvl} onClick={() => onStart(lvl)}
                className="bg-[#C84C0C] hover:bg-[#E8A060] text-white font-bold px-3 py-1 sm:px-6 sm:py-3 border-4 border-[#000] transition-colors"
                style={{ textShadow: '1px 1px 0 #000' }}>
                {lvl}-1
              </button>
            ))}
          ...
          <button onClick={onEnterEditor}
            className="flex items-center gap-2 bg-[#00A800] hover:bg-[#00C800] text-white font-bold px-4 py-1 sm:px-8 sm:py-3 border-4 border-[#000] transition-colors"
            style={{ textShadow: '1px 1px 0 #000' }}>
      ...
      <div className="hidden sm:block pointer-events-none absolute bottom-0 left-0 right-0 h-16 bg-[#C84C0C]" />
      <div className="hidden sm:block pointer-events-none absolute bottom-12 left-0 right-0 h-4 bg-[#00A800]" />
    </div>
  );
}
