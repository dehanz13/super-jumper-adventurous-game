...
describe('plasma projectile outcomes', () => {
  it('retracts a rollpod without removing it', () => {
    ...
    expect(resolvePlasmaHit(enemy)).toEqual({ points: 200, sound: null });
  ...
  it('requires multiple hits to disable a warden', () => {
    ...
    expect(resolvePlasmaHit(enemy)).toEqual({ points: 5000, sound: 'playStageClear' });
  ...
  it('removes an ordinary creature and ignores protected targets', () => {
    ...
    expect(resolvePlasmaHit(pebblit)).toEqual({ points: 200, sound: null });
