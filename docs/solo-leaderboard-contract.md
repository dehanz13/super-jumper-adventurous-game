...
- Provision the DynamoDB table and pending-outbox GSI with least-privilege IAM. Build a retry worker that sends the same match ID and idempotency key to the dedicated solo game ID. Test worker restarts against a local or scratch store before production writes.
