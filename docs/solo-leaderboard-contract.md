...
- `src/server/guestIdentity.js` now creates or resolves a guest identity for the current ISO week in UTC and passes it to run issuance. It stores only a hash of the guest credential, a random player ID, the normalized public profile, and the week boundary. The future DynamoDB adapter must conditionally insert the new session by token hash and fetch existing sessions by that hash; the future browser adapter must retain the credential in private storage and omit it from URLs, telemetry, and logs. A guest who clears browser storage can create another identity, so this is continuity for normal attempts rather than Sybil resistance. No HTTP handler or cloud store is wired yet.
- `src/server/finishRun.js` now authenticates a per-run token against its stored hash, checks the bounded finish payload, replays the campaign, and selects weekly-only or weekly-plus-all-time boards from the server-owned player class. A matching retry returns the saved result or rejection; a changed payload conflicts. The `commitVerifiedResultAndOutbox` store callback must atomically change the run from `active` to `pending_write` and create exactly one outbox item keyed by run ID. The `rejectRun` callback must conditionally save a verification rejection. These are interfaces exercised with an in-memory test store, not a deployed DynamoDB transaction or live leaderboard write.
...
- Implement the DynamoDB conditional run transitions and atomic verified-result/outbox transaction, plus a retry worker that sends the same match ID and idempotency key to the dedicated solo game ID. Test concurrent finishes and worker restarts against a local or scratch store before production writes.
