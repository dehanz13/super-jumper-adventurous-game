...
- Implement the HTTP handler with a 256 KiB raw request limit, bearer-token extraction, rate limiting, and problem-detail error mapping. The domain finish function checks canonical payload size, but the handler must reject oversized raw bodies before parsing JSON.
