...
- `src/server/issueRun.js` now issues a random run ID and bearer token only after a conditional store insert succeeds. The record stores a token hash, trusted player identity, pinned versions, a 35-minute finish deadline, and a 24-hour cleanup timestamp. It requires an identity resolved by server code; the guest session and signed Hearso handoff remain to be built. A fresh guest player ID on every attempt would create duplicate weekly rows, so guest identity must persist across attempts.
