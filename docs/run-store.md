...
The verified audit item uses the existing `due-outbox` index with `outboxStatus: audit_pending`, so the leaderboard worker does not claim it. It has no TTL because later Supabase and Kafka delivery must survive the leaderboard's shorter retry window. The event is defined by `contracts/nova-run-verified.v1.schema.json`: one event ID per run, the server-replayed score, explicit board eligibility, and pinned replay versions, with no display name, account ID, credential, or input transcript. A local `history_relay` worker mode can move the item to `audit_history_handed_off` after Supabase acknowledges its durable row; that status does not claim Kafka delivery. The Supabase table, Kafka relay, retention/deletion policy, and deployed credentials still need implementation and testing.

The local audit worker now implements conditional claims, independent destination acknowledgements, and retries using this index. Its sink contract and remaining deployment work are described in [audit delivery](audit-delivery.md). No destination adapter or scheduled invocation is connected yet.
